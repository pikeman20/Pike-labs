...
class XenForo_Helper_String
{
	...
	public static function stripCensoredText($string, array $words = null)
	{
		if ($words === null)
		{
			$words = XenForo_Application::get('options')->censorWords;
		...
		{
			if (is_string($key) || !isset($word['word']) || !isset($word['replace']))
			{
				// old format or broken
				continue;
			}

			$word['replace'] = utf8_strlen($word['word']);
		}

		return self::censorString($string, $words, '');
	}
	...
	public static function wholeWordTrim($string, $maxLength, $offset = 0, $elipses = '...')
	{
		...
		if ($maxLength > 0 && $strLength > $maxLength)
		{
			...
			{
				// too short with the elipses, can't really display anything
				$string = $elipses;
				$offset = 0;
			}
		}
	...
	public static function bbCodeStrip($string, $stripQuote = false)
	{
		...
		$string = preg_replace('#\[(attach|media|img|spoiler)(?![a-z0-9_])[^\]]*\].*\[/\\1\]#siU', "[\x1A\\1]", $string);
		...
		for ($i = 1; $i < $total; $i += 2)
		{
			if (preg_match("#^\\[/([a-z0-9_]+)]#i", $parts[$i], $match))
			{
