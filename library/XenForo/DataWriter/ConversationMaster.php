...
class XenForo_DataWriter_ConversationMaster extends XenForo_DataWriter
{
	...
	/**
	 * Constant for extra data that holds an array of user IDs that should skip receiving the conversation alert
	 *
	 * @var array
	 */
	const DATA_SKIP_ALERT = 'skipConversationAlert';
	...
	*/
	...
	protected function _postSave()
	{
		...
		if ($recipients = $this->_getUserModel()->getUsersByIds($this->_newRecipients, array('join' => XenForo_Model_User::FETCH_USER_FULL)))
		{
			...
			$skipAlerts = $this->getExtraData(self::DATA_SKIP_ALERT);
			if (!is_array($skipAlerts))
			{
				$skipAlerts = array();
			}
			...
			foreach ($recipients AS $recipient)
			{
				...
				{
					if (!in_array($recipient['user_id'], $skipAlerts))
					{
