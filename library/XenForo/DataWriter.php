...
abstract class XenForo_DataWriter
{
	...
	*/
	...
	public function getFieldNames($tableName = null)
	{
		...
		if (!empty($tableName))
		{
			...
			{
				$this->error("No fields are defined for table '" . htmlspecialchars($tableName) . "'.");
			}
	...
	*/
	public function set($field, $value, $tableName = '', array $options = null)
	{
		...
		{
			$this->error("The field '" . htmlspecialchars($field) . "' was not recognised.", $field, false);
		}
		...
		if ($updateAll)
		{
			foreach ($this->_fields AS $table => $fieldData)
			{
				foreach ($fieldData AS $fieldName => $fieldType)
				{
					...
					if ($fieldType['default'][0] == $tableName && !$this->get($fieldName, $table))
					{
	...
	*/
	...
	{
		if ($this->_triggerLanguageRebuild)
		{
