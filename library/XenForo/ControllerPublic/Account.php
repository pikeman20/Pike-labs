...
class XenForo_ControllerPublic_Account extends XenForo_ControllerPublic_Abstract
{
	...
	{
		$contactFields = $this->_getFieldModel()->getUserFields(
			array('display_group' => 'contact')
		);

		$viewParams = array(
			'isPrivacySettings' => true,
			'hasIdentities' => count($contactFields) > 0
		);
	...
	public function actionContactDetailsSave()
	{
		...
		if ($writer->isChanged('email')
			&& $writer->get('user_state') == 'valid'
			&& $writer->get('email')
			&& $writer->getExisting('email')
		)
		{
			$visitor = XenForo_Visitor::getInstance();
			$mail = XenForo_Mail::create('email_changed', array(
				'newEmail' => $writer->get('email'),
				'user' => $visitor,
				'ip' => $this->_request->getClientIp(false)
			), $visitor['language_id']);
			$mail->send($writer->getExisting('email'), $visitor['username']);
		}
	...
	{
		$this->assertPasswordConfirmed(3600); // 1 hour

		$visitor = XenForo_Visitor::getInstance();

		/** @var XenForo_Model_Tfa $tfaModel */
		$tfaModel = $this->getModelFromCache('XenForo_Model_Tfa');
		$providers = $tfaModel->getValidProviders();
		$userData = $tfaModel->getUserTfaEntries($visitor->user_id);

		/** @var XenForo_ControllerHelper_Login $loginHelper */
		$loginHelper = $this->getHelper('Login');
		$trustedKey = $loginHelper->getUserTfaTrustedKey();

		$viewParams = array(
			'providers' => $tfaModel->prepareTfaProviderList($providers, $userData),
			'backupAdded' => $this->_input->filterSingle('backup', XenForo_Input::BOOLEAN),
			'trustedRecord' => $trustedKey ? $tfaModel->getUserTrustedRecord($visitor->user_id, $trustedKey) : null,
			'otherDevicesTrusted' => $tfaModel->countUserTrustedRecords($visitor->user_id, $trustedKey)
		);
		return $this->_getWrapper(
			'account', 'two-step',
			$this->responseView('XenForo_ViewPublic_Account_TwoStep', 'account_two_step', $viewParams)
		);
	}
	...
	public function actionTwoStepEnable()
	{
		...
		$provider = $tfaModel->getValidProvider($providerId);
		$data = $tfaModel->getUserTfaEntry($visitor->user_id, $providerId);
		if (!$provider || !$provider->canEnable() || $data)
		{
			return $this->responseRedirect(
		...
		if (!$provider->meetsRequirements($user, $error))
		{
		...
		{
			if ($this->isConfirmedPost() && $this->_input->filterSingle('step', XenForo_Input::STRING) == 'setup')
			{
				$setupData = $provider->verifySetupFromInput($this->_input, $user, $error);
				if (!$setupData)
				{
					return $this->responseError($error);
				}
			...
			else
			{
				...
				return $this->_getWrapper(
					'account', 'two-step',
					$this->responseView('XenForo_ViewPublic_Account_TwoStepSetup', 'account_two_step_setup', $viewParams)
				);
			}
		}

		$session = XenForo_Application::getSession();
		$sessionKey = 'tfaData_' . $providerId;

		if ($this->isConfirmedPost() && $this->_input->filterSingle('step', XenForo_Input::STRING) == 'confirm')
		{
			...
			if (!is_array($providerData))
			{
				return $this->responseRedirect(
					XenForo_ControllerResponse_Redirect::SUCCESS,
					XenForo_Link::buildPublicLink('account/two-step', null, array('provider' => $providerId))
				);
			}

			if (!$provider->verifyFromInput('setup', $this->_input, $user, $providerData))
			{
				return $this->responseError(new XenForo_Phrase('two_step_verification_value_could_not_be_confirmed'));
			}

			$tfaModel->enableUserTfaProvider($visitor->user_id, $providerId, $providerData);
			$session->remove($sessionKey);

			$backupAdded = $tfaModel->enableBackupTfaProviderIfNeeded($user);

			return $this->responseRedirect(
				XenForo_ControllerResponse_Redirect::SUCCESS,
				XenForo_Link::buildPublicLink('account/two-step', null, array('backup' => $backupAdded ? 1 : null))
			);
		}

		$providerData = $provider->generateInitialData($user, $setupData);
		$triggerData = $provider->triggerVerification('setup', $user, $this->_request->getClientIp(false), $providerData);
		...
		return $this->_getWrapper(
			'account', 'two-step',
			$this->responseView('XenForo_ViewPublic_Account_TwoStepEnable', 'account_two_step_enable', $viewParams)
		);
	}

	public function actionTwoStepDisable()
	{
		$this->assertPasswordConfirmed(3600); // 1 hour
		...
		if ($providerId)
		{
			$provider = $tfaModel->getValidProvider($providerId);
			$data = $tfaModel->getUserTfaEntry($visitor->user_id, $providerId);
			if (!$provider || !$data || !$provider->canDisable())
			{
				return $this->responseRedirect(
					XenForo_ControllerResponse_Redirect::RESOURCE_CANONICAL,
					XenForo_Link::buildPublicLink('account/two-step')
		...
		if ($this->isConfirmedPost())
		{
			if ($providerId)
			{
				$tfaModel->disableUserTfaProvider($visitor->user_id, $providerId);
			}
			else
			{
				$tfaModel->disableTfaForUser($visitor->user_id);
			}
		...
		else
		{
			$viewParams = array(
				'providerId' => $providerId,
				'provider' => $provider,
				'title' => $provider ? $provider->getTitle() : null
			);
			return $this->_getWrapper(
				'account', 'two-step',
				$this->responseView('XenForo_ViewPublic_Account_TwoStepDisable', 'account_two_step_disable', $viewParams)
			);
		}
	}

	public function actionTwoStepManage()
	{
		$this->assertPasswordConfirmed(3600); // 1 hour
		...
		$provider = $tfaModel->getValidProvider($providerId);
		$data = $tfaModel->getUserTfaEntry($visitor->user_id, $providerId);
		if (!$provider || !$data || !$provider->canManage())
		{
			return $this->responseRedirect(
				XenForo_ControllerResponse_Redirect::RESOURCE_CANONICAL,
				XenForo_Link::buildPublicLink('account/two-step')
		...
		$result = $provider->handleManage($this, $visitor->toArray(), unserialize($data['provider_data']));
		if (!$result)
		{
		...
		else if ($result instanceof XenForo_ControllerResponse_View)
		{
	...
	public function actionNewsFeed()
	{
		...
		return XenForo_ControllerHelper_Account::wrap(
			...
			$this->responseView(
				...
				'news_feed_page',
				$this->_getNewsFeedModel()->getNewsFeedForUser($visitor->toArray(), $newsFeedId) + array(
					'canViewMemberList' => $this->_getUserModel()->canViewMemberList()
				)
			)
		);
	}
