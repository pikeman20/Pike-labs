...
class XenForo_ControllerPublic_Thread extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionShowPosts()
	{
		...
		foreach ($posts AS $key => &$post)
		{
			...
			if ($post['thread_id'] != $thread['thread_id']
				|| !$postModel->canViewPost($post, $thread, $forum)
			)
			{
				unset($posts[$key]);
				continue;
	...
	public function actionEdit()
	{
		...
		$viewParams = array(
			...
			'prefixes' => $this->_getPrefixModel()->getUsablePrefixesInForumsForced($forum['node_id'], null, $thread['prefix_id']),
	...
	public function actionEditTitle()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			if ($dwInput['prefix_id'] != $thread['prefix_id']
			...
			if (!$dw->hasErrors()
				&& $dw->get('discussion_state') == 'visible'
				&& $spamModel->visitorRequiresSpamCheck()
			)
			{
				$spamExtraParams = array(
				...
				switch ($spamModel->checkMessageSpam($dwInput['title'], $spamExtraParams, $this->_request))
				{
					case XenForo_Model_SpamPrevention::RESULT_MODERATED:
					case XenForo_Model_SpamPrevention::RESULT_DENIED;
	...
	public function actionModeratorActions()
	{
		...
		if (!$threadModel->canViewThreadModeratorLog($thread, $forum, $errorPhraseKey))
		{
			throw $this->getErrorOrNoPermissionResponseException($errorPhraseKey);
		}
	...
	public function actionMove()
	{
		...
		if ($this->isConfirmedPost()) // move the thread
		{
			...
			{
				if ($input['prefix_id'] != $thread['prefix_id']
	...
	public function actionTags()
	{
		...
		if (!$this->_getThreadModel()->canEditTags($thread, $forum, $errorPhraseKey))
		{
			throw $this->getErrorOrNoPermissionResponseException($errorPhraseKey);
		}
		...
		$editTags = $tagModel->getTagListForEdit('thread', $thread['thread_id'], $tagger->getPermission('removeOthers'));
		...
		if ($this->isConfirmedPost())
		{
			...
			if ($this->_noRedirect())
			{
				$view = $this->responseView('', 'helper_tag_list', array(
					'tags' => $cache,
					'editUrl' => XenForo_Link::buildPublicLink('threads/tags', $thread)
				));
				$view->jsonParams = array(
					'isTagList' => true,
					'redirect' => XenForo_Link::buildPublicLink('threads', $thread)
				);
				return $view;
			}
			else
		...
		else
		{
			...
			return $this->responseView('XenForo_ViewPublic_Thread_Tags', 'thread_tags', $viewParams);
		}
	}
	...
	protected function _getDefaultViewParams(array $forum, array $thread, array $posts, $page = 1, array $viewParams = array())
	{
		...
		return array(
			...
			'canEditTags' => $threadModel->canEditTags($thread, $forum),
