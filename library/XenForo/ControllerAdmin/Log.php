...
class XenForo_ControllerAdmin_Log extends XenForo_ControllerAdmin_Abstract
{
	...
	public function actionLinkProxyDetails()
	{
		$linkId = $this->_input->filterSingle('link_id', XenForo_Input::UINT);
		$link = $this->_getLinkProxyModel()->getLinkById($linkId);
		if (!$link)
		{
		...
		if (!empty(XenForo_Application::getOptions()->imageLinkProxyReferrer['enabled']))
		{
			...
			$referrers = $this->_getLinkProxyModel()->getReferrersForLink($link['link_id']);
		}
		else
		{
			$referrers = array();
		...
		return $this->responseView('XenForo_ViewAdmin_Log_LinkProxyDetails', 'log_link_proxy_details', $viewParams);
	}
	...
	public function actionImageProxyDetails()
	{
		$image = $this->_getImageOrError();

		if (!empty(XenForo_Application::getOptions()->imageLinkProxyReferrer['enabled']))
		{
			$referrers = $this->_getImageProxyModel()->getReferrersForImage($image['image_id']);
		}
		else
		{
			$referrers = array();
		...
		return $this->responseView('XenForo_ViewAdmin_Log_ImageProxyDetails', 'log_image_proxy_details', $viewParams);
	}
	...
	public function actionImageProxyView()
	{
		...
		if (!$image['use_file'])
		{
			$image = $this->_getImageProxyModel()->getImage($image['url'], true);
			if ($image)
			{
	...
	protected function _getImageOrError($imageId = null)
	{
		if ($imageId === null)
		{
		...
		return $this->_getImageProxyModel()->prepareImage($image);
	}
