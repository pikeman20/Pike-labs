...
class XenForo_Model_User extends XenForo_Model
{
	...
	public function getUsersForEmailList(array $conditions, array $fetchOptions = array())
	{
		$whereClause = $this->prepareUserConditions($conditions, $fetchOptions);
	...
	public function prepareUserConditions(array $conditions, array &$fetchOptions)
	{
		...
		if (!empty($conditions['no_empty_email']))
		{
			$sqlConditions[] = "user.email <> ''";
		}
	...
	public function ban($userId, $endDate, $reason, $update = false, &$errorKey = null, array $viewingUser = null, $triggered = false)
	{
		...
		{
			// likely a race condition, ignore it
		}
	...
	/**
	 * Initiates contact to a newly registered user with a valid state.
	 *
	 * If the $oldState == null this is a new registration.
	 * 
	 * @param array $user
	 * @param null $oldState
	 * 
	 * @return bool
	 */
	public function sendWelcomeContact(array $user, $oldState = null)
	{
		...
		switch ($oldState)
		{
			case 'moderated':
			case 'email_confirm':
			case null: // new registration

				$options = XenForo_Application::getOptions();
				$option = $options->registrationWelcome;

				if (!empty($option['messageEnabled']))
				{
					$this->_sendWelcomeMessage($user, $option);
				}

				if (!empty($option['emailEnabled']))
				{
					$this->_sendWelcomeEmail($user, $option);
				}
	...
	protected function _sendWelcomeMessage(array $user, array $option)
	{
		/** @var XenForo_Model_User $userModel */
		$userModel = XenForo_Model::create('XenForo_Model_User');
		
		$participants = explode(',', $option['messageParticipants']);
		...
		$participantUsers[$user['user_id']] = $user;
		...
		$conversationDw->set('open_invite', $option['messageOpenInvite']);
		$conversationDw->set('conversation_open', $option['messageLocked'] ? 0 : 1);
		...
		unset($participantUsers[$user['user_id']]); // we don't want to affect the new user in the following step (mark read, delete conversation)...
		$participantUsers[$starterUser['user_id']] = $starterUser; // ...we do want to affect the conversation starter though
		...
		foreach ($participantUsers AS $participant)
		{
			$conversationModel->markConversationAsRead(
				$conversationDw->get('conversation_id'), $participant['user_id'], XenForo_Application::$time
			);

			if ($option['messageDelete'] != 'no_delete')
			{
				$conversationModel->deleteConversationForUser(
					$conversationDw->get('conversation_id'), $participant['user_id'], $option['messageDelete']
				);
			}
		}
	...
	protected function _sendWelcomeEmail(array $user, array $option)
	{
		...
		if ($format == 'html')
		{
			...
			$text = trim(
				htmlspecialchars_decode(strip_tags($body))
			);
		}
		else
		{
		...
		$mail = XenForo_Mail::create('welcome_email', array(
			'title' => $title,
			'htmlBody' => $body,
			'textBody' => $text
		), $user['language_id']);
	...
	public function mergeUsers(array $target, array $source)
	{
		...
		if ($dw->setExistingData($target))
		{
			...
			// prevent situations where a user can be following/ignoring themselvs
			$this->_getDb()->query("
				DELETE FROM xf_user_follow
				WHERE user_id = ? AND follow_user_id = ?
			", array($target['user_id'], $target['user_id']));
			$this->_getDb()->query("
				DELETE FROM xf_user_ignored
