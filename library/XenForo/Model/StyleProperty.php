...
class XenForo_Model_StyleProperty extends XenForo_Model
{
	...
	public function getStylePropertiesInStyleByDefinitions($styleId, array $definitionIds)
	{
		...
		return $this->fetchAllKeyed('
			...
			WHERE style_property.style_id = ?
				AND style_property.property_definition_id IN (' . $this->_getDb()->quote($definitionIds) . ')
			ORDER BY property_definition.display_order
	...
	public function rebuildPropertyCacheInStyleAndChildren($styleId, $rebuildThisOnly = false)
	{
		...
		else
		{
			...
			if ($styleId == 0)
			{
				...
				{
					array_unshift($rebuildStyleIds, 0);
					$rebuildStyleIds[] = -1;
					$dataStyleIds[] = -1;
				}
			}
	...
	public function updatePropertyCacheInStyle($styleId, array $effectiveProperties, array $style = null)
	{
		...
		if ($styleId == 0)
		{
			...
			$this->_getStyleModel()->updateAllStylesLastModifiedDate();

			return $propertyCache;
		}

		if (!XenForo_Application::isRegistered('defaultStyleProperties'))
		{
			$defaultPropertyCache = $this->rebuildPropertyCacheInStyleAndChildren(0, true);
		...
		foreach ($defaultPropertyCache AS $key => $defaultValue)
		{
			if (!isset($propertyCache[$key]))
			{
				continue;
			}
			...
			if (is_array($defaultValue))
			{
				...
				{
					foreach ($defaultValue AS $innerKey => $innerDefaultValue)
					{
						if (!isset($localValue[$innerKey]))
						{
							// we've blanked out a value that we had by default
							$propertyCache[$key][$innerKey] = null;
						}
						else if ($localValue[$innerKey] === $innerDefaultValue)
						{
							// unchanged
					...
					if (isset($propertyCache[$key]) && count($propertyCache[$key]) == 0)
					{
	...
	public function replacePropertiesInTemplateForEditor($templateText, $editStyleId, array $properties = null, $simpleOnly = false)
	{
		...
		$templateText = preg_replace_callback(
			'/\{xen:property\s+("|\'|)(?P<propertyName>[a-z0-9_]+)(?P<propertyComponent>\.[a-z0-9._-]+)?\\1\s*\}(?![a-z0-9._-])/si',
			array('self', '_propertyToAtScalarCallback'),
			$templateText
		);
	...
	protected static function _propertyToAtScalarCallback(array $match)
	{
		...
		if (!in_array(strtolower($match['propertyName']), XenForo_DataWriter_StylePropertyDefinition::$reservedNames)
			&& isset(self::$_tempProperties[$match['propertyName']])
		)
		{
			return '@' . $match['propertyName'] . (empty($match['propertyComponent']) ? '' : $match['propertyComponent']);
		}
		else
		{
			return $match[0];
	...
	public function translateEditorPropertiesToArray($templateText, &$outputText, array $properties)
	{
		...
		foreach ($matches AS $match)
		{
			...
			$unparsableElements = preg_replace($matchRegex, '', $match['rules']);
			$unparsableElements = trim($unparsableElements);
			if ($unparsableElements)
			{
				$extra[] = $unparsableElements;
			}

			preg_match_all($matchRegex, $match['rules'], $ruleMatches, PREG_SET_ORDER);
			...
			foreach ($ruleMatches AS $ruleMatch)
			{
				...
				{
					if (isset($validComponents['extra']) && (!$propertyComponent || $propertyComponent == 'extra'))
					{
						$extra[] = $ruleMatch[0];
					}
					else
				...
				else
				{
					...
					else if (preg_match('/!important|([^a-z0-9_-]|^)inherit([^a-z0-9_-]|$)/i', $value))
					{
						$isValidRule = true;
					...
					else
					{
						...
						switch ($name)
						{
							...
							case 'border-width':
								...
								$parts = preg_split('/\s+|(\S+\([^)]*\))/', trim($value), -1, PREG_SPLIT_DELIM_CAPTURE);
	...
	public function parseBorderCss($value, $name)
	{
		...
		do
		{
			if (preg_match('/^(thin|medium|thick|0(%|[a-z]+)?|-?\d+(\.\d+)?[a-z]+)/i', $value, $match))
			{
