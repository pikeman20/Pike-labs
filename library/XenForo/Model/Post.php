...
class XenForo_Model_Post extends XenForo_Model
{
	...
	/**
	 * Fetches a list of posts made by $userId
	 * except those which are the first post of a thread.
	 *
	 * @param $userId
	 *
	 * @return array
	 */
	public function getPostsByUserForSpamCleaner($userId)
	{
		return $this->fetchAllKeyed('
			SELECT post.*
			FROM xf_post AS post
	...
	protected function _moveOrCopyPosts($action, array $posts, array $sourceThreads, array $targetThread, array $options = array())
	{
		...
		$sourceThreadIds = array_keys($sourceThreadIds);
		...
		else
		{
			...
			foreach ($posts AS $post)
			{
				$sourceThread = isset($sourceThreads[$post['thread_id']])
					? $sourceThreads[$post['thread_id']] : null;
				$sourceForum = $sourceThread && isset($sourceForums[$sourceThread['node_id']])
					? $sourceForums[$sourceThread['node_id']] : null;

				$sourceCounts = (
					$sourceThread && $sourceForum
					&& $sourceThread['discussion_state'] == 'visible'
					&& $post['message_state'] == 'visible'
					&& $post['user_id']
					&& !empty($sourceForum['count_messages'])
				);
				$targetCounts = (
